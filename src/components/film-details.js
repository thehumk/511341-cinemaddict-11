...
export default class FilmDetails {
  ...
  getTemplate() {
    let genresFilm = ``;
    for (let i = 0; i < this._film.genre.length; i++) {
      genresFilm += `<span class="film-details__genre">${this._film.genre[i]}</span>`;
    }
    const genreTitle = this._film.genre.length <= 1 ? `Genre` : `Genres`;
